...
<div id="app" hidden><section class="panel stack"><div><h1>把声音变成文字</h1><p class="muted" id="limits">选择手机里的视频、录音或字幕。</p></div><label class="pick"><span>＋ 选择视频、录音或字幕</span><input id="file" type="file" accept=".mp4,.mkv,.webm,.mov,.mp3,.wav,.m4a,.flac,.srt,.vtt" aria-label="选择视频、录音或字幕"></label><p id="filename" class="muted">尚未选择文件</p><button id="upload" disabled>开始转文字</button><div id="upload-progress" hidden><progress id="upload-bar" max="100" value="0" aria-label="上传进度"></progress><p id="upload-state"></p></div></section>
...
function logout() {
  ...
  selected=null; jobs=[]; resultFor=""; $("key").value=""; $("file").value="";
...
async function api(path, options={}) {
  ...
  if(!response.ok) throw failure(response.status,response.headers.get("Retry-After"));
  return response;
}
